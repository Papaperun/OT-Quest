...
"""
Turbidity Meter Simulator with REAL Modbus TCP writer

Modified to write to actual Modbus server (your Pi!)
"""
...
# Import real pymodbus
from pymodbus.client import ModbusTcpClient

# -------------- Configuration --------------
MODBUS_HOST = "192.168.1.184" 
MODBUS_PORT = 5020
REGISTER_ADDR = 2       # Using register 2 (address starts at 0)
...
# -------------- REAL Modbus Writer --------------
class ModbusWriter:
    def __init__(self, host, port, register_addr):
        self.host = host
        self.port = port
        self.register = register_addr
        self.client = ModbusTcpClient(host, port=port)
        self.connected = False
        print(f"[Modbus] Initializing connection to {host}:{port}")
    ...
    def write_ntu(self, ntu):
        """Write NTU value scaled by 100 (e.g., 0.45 NTU -> 45)"""
        if not self.connected:
            print(f"⚠️ Not connected, attempting reconnect...")
            self.connect()
            if not self.connected:
                return False
        ...
        try:
            ...
            result = self.client.write_register(self.register, value, slave=1)
            
            if not result.isError():
                print(f"✅ Wrote: {ntu:.2f} NTU (value={value}) to register {self.register}")
                return True
            else:
...
class App:
    def __init__(self, root, sim: TurbiditySimulator):
        self.root = root
        self.sim = sim
        self.root.title("Turbidity Meter Simulator — Connected to Pi!")
    ...
    def setup_ui(self):
        ...
        tk.Label(title_frame, text="🌊 TURBIDITY MONITOR", font=("Arial", 18, "bold"),
        ...
        # Status Display
        self.label_status = tk.Label(self.root, text="● SYSTEM NORMAL", 
                                     font=("Arial", 12, "bold"),
                                     bg="#1a1a2e", fg="#00ff88", pady=6)
        self.label_status.pack()
        
        # Modbus Status
        self.modbus_label = tk.Label(self.root, text="Modbus: Connecting...", 
                                     font=("Arial", 9),
                                     bg="#1a1a2e", fg="#4ecdc4")
        ...
        self.btn_normal = tk.Button(btn_frame, text="⚡ Normal Ops", command=self.set_normal,
                                    width=14, font=("Arial", 10, "bold"),
                                    bg="#00ff88", fg="#1a1a2e", relief="flat",
                                    activebackground="#00cc6a")
        self.btn_normal.pack(side="left", padx=5)
        
        self.btn_storm = tk.Button(btn_frame, text="🌧️ Storm Weather", command=self.set_storm,
                                     width=14, font=("Arial", 10, "bold"),
                                     bg="#4a5568", fg="white", relief="flat",
                                     activebackground="#2d3748")
        self.btn_storm.pack(side="left", padx=5)
        
        self.btn_manual = tk.Button(btn_frame, text="✋ Manual Entry", command=self.set_manual,
                                    width=14, font=("Arial", 10, "bold"),
                                    bg="#4a5568", fg="white", relief="flat",
                                    activebackground="#2d3748")
    ...
    def _update_ui(self):
        ...
        dots = "." * min(int(ntu * 100), 200)  # Cap at 200 dots
        self.pixel_display.config(text=f"[ {dots} ]")
        
        if ntu >= 1.0:
            self.label_status.config(text="⚠️ EMERGENCY SHUTDOWN", fg="#ff0000")
        elif ntu >= 0.30:
            self.label_status.config(text="⚠️ HIGH TURBIDITY WARNING", fg="#ffa500")
        else:
            self.label_status.config(text="● SYSTEM NORMAL", fg="#00ff88")
        
        modbus_text = "Modbus: ✅ Connected" if ok else "Modbus: ❌ Error"
...
# -------------- Main --------------
def main():
    # Create REAL Modbus writer
    modbus = ModbusWriter(MODBUS_HOST, MODBUS_PORT, REGISTER_ADDR)
    
    # Try to connect
    if not modbus.connect():
        print("WARNING: Could not connect to Modbus server!")
        print(f"Make sure server is running at {MODBUS_HOST}:{MODBUS_PORT}")
